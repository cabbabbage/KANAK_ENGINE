...
SDL_Texture* RenderAsset::render_shadow_mask(Asset* a, int bw, int bh) {
    ...
    Uint8 light_alpha = static_cast<Uint8>(main_light_source_.get_brightness());
    render_shadow_received_static_lights(a, bounds, light_alpha);
    render_shadow_moving_lights(a, bounds, light_alpha);

    Uint8 main_alpha = main_light_source_.get_current_color().a;
...
SDL_Texture* RenderAsset::regenerateFinalTexture(Asset* a) {
    ...
    if (a->has_shading) {
        SDL_Texture* mask = render_shadow_mask(a, bw, bh);
        if (mask) {
            SDL_SetRenderTarget(renderer_, final_tex);
...
void RenderAsset::render_shadow_moving_lights(Asset* a,
    ...
    for (const auto& light : p->info->light_sources) {
        ...
        double factor = LightUtils::calculate_static_alpha_percentage(a, p);
...
void RenderAsset::render_shadow_orbital_lights(Asset* a,
                                               const SDL_Rect& bounds,
                                               Uint8 alpha) {
    if (!a || !a->info) return;
...
void RenderAsset::render_shadow_received_static_lights(Asset* a,
                                                       const SDL_Rect& bounds,
                                                       Uint8 alpha) {
    if (!a) return;
