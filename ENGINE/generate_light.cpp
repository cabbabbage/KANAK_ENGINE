...
SDL_Texture* GenerateLight::generate(const Asset* asset, const LightSource& light) {
    if (!renderer_ || !asset || !asset->info)
        return nullptr;
        ...
        for (int x = 0; x < size; ++x) {
            ...
            for (int s = 0; s < 4; ++s) {
                ...
                for (const auto& [ray_angle, spread] : rays) {
                    ...
                    if (diff < spread)
                        ray_boost += (1.0f - (diff / spread)) * 0.05f;
                }
                ...
                accum_r += final_color.r;
                accum_g += final_color.g;
                accum_b += final_color.b;
                accum_a += final_color.a;
            }

            SDL_SetRenderDrawColor(renderer_,
                static_cast<Uint8>(accum_r / 4.0f),
                static_cast<Uint8>(accum_g / 4.0f),
                static_cast<Uint8>(accum_b / 4.0f),
                static_cast<Uint8>(accum_a / 4.0f)
            );
