...
public:
    ...
    void mergeDistantAssets(const std::vector<std::vector<Asset*>>& groups);
...
private:
    size_t validateAndRemoveInvalidTextures();
    ...
    // Utility: get all room & trail areas
    std::vector<Area> getRoomTrailAreas() const; // alias of getAllRoomAndTrailAreas if needed
};
