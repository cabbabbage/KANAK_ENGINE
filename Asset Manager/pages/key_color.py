...
class ColorKeyCircleEditor(tk.Canvas):
    def __init__(self, parent, on_change=None, width=300, height=300, **kwargs):
    ...
    def _ensure_fixed_keys_exist(self):
        ...
        if  0.0 not in present: self.keys.append({"angle":0.0,   "color":(255,255,255,255)})
        if  180.0 not in present: self.keys.append({"angle":180.0, "color":(0,  0,  0,  255)})
    ...
    def _on_drag(self, e):
        if not self.selected_key or self.selected_key["angle"] in self.fixed_angles: return
    ...
    def _open_context_menu(self, key):
        if key["angle"] in self.fixed_angles: return
    ...
    def _change_color(self, key):
        init = '#%02x%02x%02x'%key["color"][:3]
        col  = colorchooser.askcolor(initialcolor=init)[0]
        if col:
            key["color"] = (int(col[0]),int(col[1]),int(col[2]),255)
