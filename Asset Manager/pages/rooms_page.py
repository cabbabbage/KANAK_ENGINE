...
class RoomsPage(ttk.Frame):
    ...
    def load_data(self, data=None, json_path=None):
        ...
        files = sorted([f for f in os.listdir(self.rooms_dir) if f.endswith(".json")])
        if files:
            first_path = os.path.join(self.rooms_dir, files[0])
            try:
                with open(first_path) as f:
                    self.room_data = json.load(f)
                    self.current_room_path = first_path
                    self._load_editor()
    ...
    def _build_editor(self):
        ...
        self.width_range = Range(self.editor_frame, min_bound=10, max_bound=1000, label="Room Width")
        ...
        self.height_range = Range(self.editor_frame, min_bound=10, max_bound=1000, label="Room Height")
        self.height_range.pack(fill=tk.X, pady=6)
        self.height_range.var_min.trace_add("write", lambda *_: self._save_json())
        self.height_range.var_max.trace_add("write", lambda *_: self._save_json())

        self.edge_smoothness = Range(self.editor_frame, min_bound=0, max_bound=1, label="Edge Smoothness", force_fixed=True)
        self.edge_smoothness.pack(fill=tk.X, pady=6)
        self.edge_smoothness.var_min.trace_add("write", lambda *_: self._save_json())
        self.edge_smoothness.var_max.trace_add("write", lambda *_: self._save_json())

        ttk.Label(self.editor_frame, text="Geometry:").pack(anchor="w", pady=(8, 0))
        self.geometry_var = tk.StringVar(value="Random")
        ...
        self.geometry_dropdown.bind("<<ComboboxSelected>>", lambda _: self._save_json())

        self.spawn_var = tk.BooleanVar()
        self.boss_var = tk.BooleanVar()

        def handle_checkboxes(*_):
            if self.spawn_var.get() and self.boss_var.get():
                if self._last_changed == "spawn":
        ...
        self.asset_editor = AssetEditor(
            ...
            positioning=True,
            current_path=self.current_room_path
        )
    ...
    def _load_editor(self):
        self.name_var.set(self.room_data["name"])
