...
class ChildAssetsPage(ttk.Frame):
    def __init__(self, parent):
        ...
        # — Layout —
        ttk.Label(self, text="Region-Spawned Children",
        ...
        self.container.grid(row=1, column=0, columnspan=5,
                            sticky="nsew", padx=12)
        self.columnconfigure(1, weight=1)

        # Add Child button
        ttk.Button(self, text="Add Child Region",
                   style='Main.TButton',
                   command=self._add_child_row)\
            .grid(row=2, column=0, columnspan=5, pady=(0,12))

        # Save button
        ttk.Button(self, text="Save",
                   style='Main.TButton',
                   command=self.save)\
            .grid(row=3, column=0, columnspan=5, pady=(0,12))

    def _add_child_row(self, data=None):
        idx = len(self.child_frames)
        frm = ttk.Frame(self.container.scrollable_frame,
                        relief='ridge', padding=12)
        frm.grid(row=idx, column=0, sticky="ew", pady=8)
        ...
        # Terminate with Parent checkbox
        term_var = tk.BooleanVar(value=(data or {}).get('terminate_with_parent',False))
        ttk.Checkbutton(frm, text="Terminate with Parent",
                        variable=term_var,
                        style='Large.TCheckbutton')\
            .grid(row=5, column=0, columnspan=4,
                  sticky="w", pady=(8,4))

        # Preview of selected child asset
        cv = tk.Canvas(frm, width=400, height=400, bg='black')
        cv.grid(row=0, column=4, rowspan=6, padx=10, pady=4)
    ...
    def load(self, info_path):
        ...
        self.asset_names = [
            ...
            if n!=current and
               os.path.isdir(os.path.join(ASSET_DIR,n)) and
               json.load(open(os.path.join(ASSET_DIR,n,"info.json"))).get("child_only",False)
        ]
        # clear
    ...
    def save(self):
        ...
        old = []
        try:
            old = [c.get('area_file') for c in json.load(open(self.asset_path)).get('child_assets',[])]
        except: pass
        used = set()
        out = []
